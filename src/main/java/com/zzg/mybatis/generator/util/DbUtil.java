...
//		executorService.shutdown();
    ...
    public static Connection getConnection() throws ClassNotFoundException, SQLException {
    	...
    	if(dbConfig==null) {
    	    ...
    	    props.setProperty("user", dbConfig.getUsername()); //$NON-NLS-1$
    	    props.setProperty("password", dbConfig.getPassword()); //$NON-NLS-1$
	...
	public static String getColumnRemark(String tableName, String colName) throws Exception {
		...
		try {
			...
			if (DbType.valueOf(dbConfig.getDbType()) == DbType.SQL_Server) {
				String sql = "SELECT cast([value] as nvarchar) FROM ::fn_listextendedproperty(NULL, 'user', 'dbo', 'table', '" + tableName
						+ "', 'column', '" + colName + "')";
				rs = connection.createStatement().executeQuery(sql);
				while (rs.next()) {
...
//				throw new NullPointerException("DB類型 : "+dbConfig.getDbType()+"  取得remark方式尚未設定")
