...
public class DbRemarksCommentGenerator implements CommentGenerator {
    ...
    private Map<String,Boolean> pkFieldMap = new HashMap<String,Boolean>();
...
//            compilationUnit.addImportedType(new FullyQualifiedJavaType("org.hibernate.validator.constraints.NotEmpty"));
    ...
    public void addModelClassComment(TopLevelClass topLevelClass,
        ...
        topLevelClass.addJavaDocLine(" */");
        if(isAnnotations) {
            topLevelClass.addAnnotation("@Table(name=\"" + introspectedTable.getFullyQualifiedTable().getIntrospectedTableName().toLowerCase() + "\")");
        }
    }
    ...
    public void addFieldComment(Field field,
		...
		if (columnRemarks) {
			String remark = null;
			try {
				remark = DbUtil.getColumnRemark(introspectedTable.getFullyQualifiedTable().getIntrospectedTableName(),
        ...
        if (isAnnotations) {
			...
			for (IntrospectedColumn column : introspectedTable.getPrimaryKeyColumns()) {
				if (introspectedColumn == column) {
					//PK類別不配置@Id
					if (introspectedTable.getPrimaryKeyColumns().size()==1 || pkFieldMap.containsKey(column.getActualColumnName())) {
						isId = true;
						field.addAnnotation("@Id");
						field.addAnnotation("@GeneratedValue");
						pkFieldMap.remove(column.getActualColumnName());
					} else {
						isId = true;
						pkFieldMap.put(column.getActualColumnName(), true );
					}
					break;
				}
			}
            ...
            field.addAnnotation("@Column(name = \""+introspectedColumn.getActualColumnName().toLowerCase()+"\")");
